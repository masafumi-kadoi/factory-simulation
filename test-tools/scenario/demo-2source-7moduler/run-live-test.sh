...
SPEED="${1:-10.0}"
ZIP_NAME="demo-2source-7moduler.zip"
...
if [ -z "$DS_ID" ]; then
    ...
    SCENARIO_ID=$(echo "$SCENARIO_RESP" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('scenarioId',''))" 2>/dev/null || echo "")
...
for s in (scenarios if isinstance(scenarios, list) else []):
    if s.get('name') == 'Demo-2Source-7Moduler-Merge':
        print(s.get('scenarioId', s.get('id', '')))
        break
" 2>/dev/null || echo "")
    fi
    ...
    DS_RESP=$(curl -sk -X POST "$GATEWAY_URL/api/data-sources" \
        -H 'Content-Type: application/json' \
        -d "{\"friendlyName\": \"demo-2source-7moduler-live\", \"scenarioId\": \"$SCENARIO_ID\", \"sourceType\": \"simulation\"}")
